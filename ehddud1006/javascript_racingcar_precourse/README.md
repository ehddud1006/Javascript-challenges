...
|기능|✅|
|---|-|
|자동차이름을 입력 후 Enter 키 또는 확인 버튼 클릭 시 입력 데이터를 저장합니다.|✔️|
|사용자가 입력한 자동차이름이 유효한 값인지 판단하고, 각각의 자동차를 배열에 저장합니다.|✔️|
|경주 시도횟수를 입력합니다.|✔️|
|사용자가 입력한 시도횟수가 유효한 값인지 판단합니다.||
|0-9의 숫자를 뽑는 메소드를 구현합니다.||
|랜덤숫자의 값에 따라 자동차를 전진, 정지시킵니다.||
|전진을 가장 많이한 자동차를 출력합니다.||
|게임 종료 후 다시 시작하는 기능||
|스크립트 모듈화||
|테스트 실행||
|상수화 시키기||
...
## 🧐 순서도
![제목 없는 다이어그램 drawio (4)](https://user-images.githubusercontent.com/62373865/197516573-7311149a-f956-4ce4-9c4d-057c854b496c.png)
